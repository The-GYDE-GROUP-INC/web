...
function ProfileForm() {
  ...
  const validationSchema = Yup.object({
    ...
    companyName: Yup.string().required(
      t("onboarding.profile.companyName-error"),
    ),
    profile: Yup.mixed()
      .nullable()
      ...
      {({
        ...
        <Form>
          <div className="flex items-center min-h-screen py-20 justify-center bg-neutral-100">
            <Card
              variant="shadow"
              className="w-96 overflow-hidden !pt-0 !px-0 pb-6"
            >
              ...
              <div className="relative flex items-end pb-10 justify-center bg-profile-auth h-52 bg-cover bg-no-repeat after:absolute after:inset-0 after:bg-black/30">
                <button
                  type="button"
                  onClick={back}
                  className="absolute text-white z-10 top-10 left-4 flex items-center gap-2"
                >
                  <ChevronLeft size={24} />
                  {t("profile")}
                </button>
                <div
                  className={cn(
                    "relative z-10 w-[90px] h-[90px] flex items-center justify-center rounded-full",
                    values.profile ? "" : "border bg-actual-grey",
                  )}
                >
                  {values.profile ? (
                    <Image
                  ...
                  <label
                    ...
                    className={cn(
                      ...
                      values.profile
                        ? "bg-light-primary text-primary hover:bg-primary hover:text-white"
                        : "bg-actual-grey text-gray-400 hover:bg-white border-divider",
                    )}
                  >
                    {values.profile ? (
                      <Pencil size={16} />
              ...
              <div className="pt-6 px-6">
                ...
                <Text className="text-light-grey">
                  {t("onboarding.profile.profile-subtitle")}
                </Text>
                ...
                <div className="space-y-4 mt-6">
                  {[
                    ...
                    "onboarding.profile.companyName",
                  ].map((field) => (
                    <div key={field}>
                      <label
                        className="block text-dark-grey mb-2 text-[15px] font-medium"
                        htmlFor={field}
                      >
                        {t(field)}
                      </label>
                      <Input
                        ...
                        onBlur={handleBlur}
                        isError={Boolean(touched[field] && errors[field])}
                      />
                      <ErrorMessage
                        name={field}
                        component="div"
